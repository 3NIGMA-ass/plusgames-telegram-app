...
export async function reinvestDeposit(
  telegram_id: string,
  amount: number,
  ...
  try {
    const result = await query<ReinvestResult>(queryText, [
      telegram_id,
      amount,
    ]);
...
export async function saveAccum(
  telegram_id: string,
  amount: number,
): Promise<{ user: User; depositEarnings: DepositEarning[] }> {
  try {
      ...
      insert_earning AS (
        ...
        WHERE can_add_earning = TRUE
          AND NOT EXISTS (
            ...
            WHERE telegram_id = $1 AND last_collected_at IS NULL
          )
...
export async function updateAccum(
  telegram_id: string,
  amount: number,
): Promise<{ user: User; depositEarnings: DepositEarning[] }> {
  try {
    ...
    if (!result.rows[0]) {
      ...
      if (
        !last_deposit_at ||
        new Date(last_deposit_at) > new Date(Date.now() - 24 * 60 * 60 * 1000)
      ) {
        throw new Error(
          'Сбор возможен только через 24 часа после последнего депозита',
        );
      }
...
`.trim();
    ...
    const inlineKeyboard = [
      [
        {
          text: '✅ Подтвердить',
          callback_data: `approve_deposit_${transactionId}`,
        },
        {
          text: '🔄 Без рефки',
          callback_data: `approve_deposit_no_ref_${transactionId}`,
        },
        {
          text: '❌ Отклонить',
          callback_data: `cancel_transaction_${transactionId}`,
        },
      ],
    ];
  ...
  try {
    const checkResult = await query<ValidationResultDep>(
      `SELECT CASE
         ...
         WHEN (SELECT balance FROM users WHERE telegram_id = $1) < 100 
           AND EXISTS (
             SELECT 1 FROM transactions 
             WHERE telegram_id = $1 
             AND type = 'withdrawal'
           ) THEN 'Баланс менее 100 RUB и есть заявка на вывод'
         WHEN (SELECT balance FROM users WHERE telegram_id = $1) < $2 THEN 'Недостаточно средств на балансе'
         WHEN $2 < 100 THEN 'Минимальная сумма вывода 100 RUB'
         ELSE 'OK'
       END AS validation_result`,
      [telegram_id, amount],
    );
    ...
    const inlineKeyboard = [
      [
        {
          text: '✅ Подтвердить',
          callback_data: `approve_withdrawal_${transactionId}`,
        },
        {
          text: '❌ Отклонить',
          callback_data: `cancel_transaction_${transactionId}`,
    ...
    const isCrypto = ['bitcoin', 'ton', 'usdt'].includes(
      method_id.toLowerCase(),
    );
