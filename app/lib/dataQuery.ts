    ...
    const result = await query<{
      ...
      `SELECT 
        ...
        (SELECT json_agg(ac) FROM (
          SELECT id, manager_link, referral_percent, useapiforcard, api_amount_ranges, manager_only_threshold FROM app_config
        ) ac) AS app_config
       FROM users u
...
export async function getTransactionsByTelegramId(
  telegramId: bigint,
): Promise<Transaction[]> {
  try {
...
export async function getPartnerEarningsByTelegramId(
  telegramId: bigint,
...
export async function getDepositEarningsByTelegramId(
  telegramId: bigint,
): Promise<DepositEarning | null> {
  try {
...
export async function getReinvestmentsByTelegramId(
  telegramId: bigint,
