...
export function WithdrawalRequestDialog({
  ...
  const isLowBalanceRestricted = userBalance < 100 && hasWithdrawalRequest;
  ...
  useEffect(() => {
    ...
    } else if (amount && userBalance < 100 && hasWithdrawalRequest) {
      newErrors.amount = `Баланс менее 100 RUB и есть заявка на вывод`;
    }
    if (
      isCrypto &&
      walletAddress &&
  ...
  const validateForm = () => {
    ...
    if (isLowBalanceRestricted) {
      newErrors.form =
        'Вывод недоступен: баланс менее 100 RUB и есть заявка на вывод';
    }
    if (isWithin24Hours) {
    ...
    if (!validateForm()) {
      if (isLowBalanceRestricted) {
        showNotification(
          'Вывод недоступен',
          'info',
          'Баланс менее 100 RUB и есть заявка на вывод',
        );
